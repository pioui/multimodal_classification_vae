...
class RelaxedSVAE(nn.Module):
    ...
    x -EncoderB-> z_1 -ClassifierA-> (y)ys -EncoderA-> z_2 -DecoderA-> z_1s
                        |                       |        
                        -------------------------    
                        --BernoulliDecoderA-> xs
    ...
    def inference(
        ...
        qc_z1 = self.classifier[encoder_key](z1) # Probabilities for each class
        log_qc_z1 = qc_z1.log()
        qc_z1_all_probas = qc_z1

        # C
        if y is None: # Unsupervised mode
            if reparam:
        ...
        log_qc_z1 = torch.gather(log_qc_z1, dim=-1, index=y_int.unsqueeze(-1)).squeeze(-1)
        ...
        variational_density = log_qz1_x + log_qz2_z1 #should it be +log_qc_z1
        ...
        variables = dict(
            ...
            log_qz1_x=log_qz1_x,   
            qc_z1=qc_z1,            # Probabilities for each class given z1
            log_qc_z1=log_qc_z1,    # log Probabilities for each class given z1
            log_qz2_z1=log_qz2_z1,
            log_pz2=log_pz2,        # the distance of N(0,1) from z2 distribution
            log_pc=log_pc,          # the distance of prior distribution from the original or classifier labels distribution
            pc=pc,
            log_pz1_z2=log_pz1_z2,  # the diastance of pz1_z2 from the z1 distribution
            log_px_z=log_px_z,      # the diastance of px_z from the x distribution
