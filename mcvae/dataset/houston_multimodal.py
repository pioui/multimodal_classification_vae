...
class houstonMultimodalDataset(Dataset):
    def __init__(
        ...
        self.train_dataset = TensorDataset(x_train[:,:50],x_train[:,50:], y_train-1) 
        self.train_dataset_labelled = TensorDataset(x_train_labelled[:,:50],x_train_labelled[:,50:], y_train_labelled-1) 
        self.test_dataset = TensorDataset(x_test[:,:50],x_test[:,50:], y_test-1) 
        self.test_dataset_labelled = TensorDataset(x_test_labelled[:,:50],x_test_labelled[:,50:], y_test_labelled-1) 
        self.full_dataset = TensorDataset(x_all[:,:50],x_all[:,50:], y_all) 
