...
class HoustonDataset(Dataset):
    def __init__(
        ...
        x = torch.Tensor(scipy.io.loadmat(os.path.join(data_dir,'houston_dataset.mat'))['features']) # [1905, 349, 151]
        x = torch.transpose(x, 1,0) # [349, 1905, 151]        
        print(x.shape)

        x_mask = torch.Tensor(scipy.io.loadmat(os.path.join(data_dir,'houston_selected.mat'))['indices']) # [1905, 349, 151] 
        x_mask = torch.transpose(x_mask, 1,0) # [349, 1905, 151]       \
        print(x_mask.unique())
        print(x_mask.shape)

        y = torch.Tensor(np.load(os.path.join(data_dir,'groundtruth.npy'))) # [349, 1905] 0 to 15
        print(y.shape)
        print(y.unique())

        x_all = x.reshape(-1,x.shape[-1])
        y_all = y.reshape(-1)
        x_mask = x_mask.reshape(-1,x_mask.shape[-1])
        print(x_all.shape)
        print(y_all.shape)
